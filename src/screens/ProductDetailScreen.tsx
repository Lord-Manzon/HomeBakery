...
import { getCategoryVisual } from '../utils/productCategoryIcon';
import { ErrorBanner } from '../components/ErrorBanner';
import { PrimaryButton } from '../components/PrimaryButton';
import { VariantFormSheet } from '../components/VariantFormSheet';
import { ConfirmDialog } from '../components/ConfirmDialog';
import { Screen } from '../components/Screen';
import { uploadProductPhoto } from '../services/products';
import { formatCurrency } from '../utils/currency';
import { spacing, radii, typography, motionDuration, motionEasing, motionStagger } from '../theme';
import type { ColorToken } from '../theme/colors';
import type { ProductCategory, ProductVariant } from '../types/product';

/**
 * Shared implementation used by every route that can open a product's
 * detail screen — the Products tab's own `[id]/index.tsx`, plus a thin
 * wrapper nested under Recipes (`[id]/product-view`) reached from a
 * recipe's "Used in" list. Same reasoning as RecipeDetailScreen.tsx's
 * own top comment: `recipeAndCostingPath` is passed in fully-formed by
 * each wrapper rather than built here from a shared "basePath" string,
 * since concatenating further segments onto a path that already has a
 * `?query` in it silently breaks (see RecipeDetailScreen.tsx's comment
 * for the specific bug that caused).
 */
export function ProductDetailScreen({
  productId,
  recipeAndCostingPath,
}: {
  productId: string;
  /** Exact path to this variant's Recipe & Costing screen. When already
   * inside the Products tab this is just the tab's own native route;
   * from Recipes (via product-view) it currently still jumps tabs —
   * see product-view.tsx's comment for why that's left unfixed here. */
  recipeAndCostingPath: (variantId: string) => string;
}) {
  const id = productId;
  ...
  const handleRecipeAndCosting = () => {
    ...
    router.push(recipeAndCostingPath(defaultVariant.id));
  };
