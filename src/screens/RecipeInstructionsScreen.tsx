import { useHideFloatingNav } from '../hooks/useHideFloatingNav';
 ...
 */
/**
 * Shared implementation used by every route that can open this recipe's
 * instructions editor — same reasoning as RecipeDetailScreen.tsx's own
 * comment. This screen's only navigation is `router.back()`, so unlike
 * RecipeDetailScreen it needs nothing beyond `recipeId` itself — no
 * basePath, since it never links further forward.
 */
export function RecipeInstructionsScreen({ recipeId }: { recipeId: string }) {
  useHideFloatingNav();
  const id = recipeId;
